...
// #car lives inside #section1 so it is anchored to the hero on both desktop
// and mobile. On desktop GSAP scrubs it across the page; on mobile CSS keeps
// it absolute inside this section so it scrolls away naturally.
...
export default function Hero() {
  return (
    <section
      id="section1"
      className="relative h-screen grid grid-cols-1 md:grid-cols-[55%_45%] items-start md:items-center pt-20"
      style={{
        background: `
    ...
    >
      ...
      {/* ── Left column ── desktop car visual space ───────────────────────── */}
      ...
      <div className="flex flex-col items-center md:items-start text-center md:text-left px-8 md:pr-16 md:pl-4 py-8 pb-[48vh] md:pb-0">

        <p className="hero-eyebrow hidden md:block font-condensed text-[0.75rem] font-bold tracking-[5px] uppercase text-brand-yellow mb-6 opacity-0 translate-y-5">
      ...
      {/* ── Mobile car zone — road lines + ambient glow behind the car ─────── */}
      <div className="md:hidden absolute bottom-0 left-0 right-0 h-[45vh] flex items-end justify-center pb-4">
        <div className="absolute bottom-[12vh] left-0 right-0">
          <div className="h-[2px] bg-linear-to-r from-transparent via-[rgba(255,208,0,0.25)] to-transparent" />
          <div className="flex gap-3 px-6 mt-1">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-[2px] flex-1 bg-[rgba(255,208,0,0.12)] rounded-full" />
            ))}
          </div>
        </div>
        <div className="absolute bottom-[8vh] left-1/2 -translate-x-1/2 w-[60vw] h-[20vh] rounded-full bg-[radial-gradient(circle,rgba(255,208,0,0.06)_0%,transparent_70%)] pointer-events-none" />
      </div>
